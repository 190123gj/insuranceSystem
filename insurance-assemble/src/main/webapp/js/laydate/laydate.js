...
;!function (win) {
    ...
    function inputChange() {
        ...
        if (Dates.elem.fireEvent) {
            Dates.elem.fireEvent('onchange');
        } else {
            Dates.elem.dispatchEvent(ev);
        }
    }
    //修复时间改变后无 change 事件 end
    ...
    Dates.creation = function (ymd, hide) {
        ...
        // 修复选择后不能触发 change 事件 start
        inputChange();
        // 修复选择后不能触发 change 事件 end
        if (!hide) {
            Dates.close();
    ...
    Dates.events = function () {
        ...
        Dates.on(as.otoday, 'click', function () {
            var now = new Date();
            if (!!Dates.checkVoid(now.getFullYear(), now.getMonth() + 1, now.getDate()).length) {
                return
            }
